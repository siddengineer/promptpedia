...
content creation, coding, career, marketing, business, education, writing,
data & analytics, health, finance, personal development, design, sales, and
prompt engineering itself). Built with **FastAPI** on the backend and plain
...
│   └── data/
│       ├── prompts.json   # 500 prompts (generated)
│       └── gen.py         # script that generated prompts.json
├── frontend/
│   ├── index.html
│   ├── style.css
│   └── script.js
...
Then open **http://localhost:8000** in your browser. That's it — FastAPI
serves both the API and the static frontend from the same origin, so there's
no CORS setup needed for local use.

## API reference

| Endpoint | Description |
|---|---|
| `GET /api/health` | Health check + total prompt count |
| `GET /api/stats` | Totals + per-category counts |
| `GET /api/categories` | List of all categories |
| `GET /api/frameworks` | List of all prompt frameworks (RTF, AIDA, STAR, etc.) |
| `GET /api/prompts` | Paginated, searchable, filterable list. Query params: `q`, `category`, `framework`, `page`, `page_size` |
| `GET /api/prompts/{id}` | A single prompt by id |
| `GET /api/random` | One random prompt (used by the "Shuffle" button) |

Interactive API docs are available automatically at `/docs` (Swagger UI) and
`/redoc`.

## Adding more prompts

Edit `backend/data/gen.py` — add topics to any category's list — then re-run:
...
This regenerates `prompts.json`. Restart the server (or just refresh if
using `--reload`) to pick up the new cards.

## Frontend notes

- No frameworks or build tools — just static files served by FastAPI.
- Design concept: a physical library card catalog. Categories are "drawers,"
  each prompt is an "index card," and frameworks (RTF, AIDA, STAR, SMART...)
