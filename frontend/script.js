...
async function init() {
  ...
  els.statTotal.textContent = stats.total_prompts;
  els.statCats.textContent = stats.total_categories;
  els.statFrameworks.textContent = stats.total_frameworks;
  renderDrawers(stats.per_category);

  const fw = await fetchJSON(`${API}/frameworks`);
  renderFrameworkChips(fw.frameworks);
...
function bindEvents() {
  ...
  els.searchInput.addEventListener("input", (e) => {
    ...
    debounceTimer = setTimeout(loadPrompts, 280);
  });
  ...
  els.shuffleBtn.addEventListener("click", async () => {
    const card = await fetchJSON(`${API}/random`);
    openCard(card);
  });
...
async function openContributeForm() {
  const catData = await fetchJSON(`${API}/categories`);
  const options = catData.categories
...
async function loadPrompts() {
  els.resultsLabel.textContent = "Searching the catalog…";
...
function renderPager() {
  ...
  let prev = 0;
  [...pages].filter((n) => n >= 1 && n <= total).sort((a, b) => a - b).forEach((n) => {
    if (prev && n - prev > 1) {
